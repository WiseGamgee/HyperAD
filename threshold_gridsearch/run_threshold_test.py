    ...
    for name, detector in tqdm(detector_dict.items(), disable=True):
        ...
        for i in range(n_repeats):
            ...
            sim = DetectionSimulator(sampler=line_sampler,
                                     detector=detector)
            detection_results, _ = sim.run(disable_progress_bar=False)

        # Add detection results for current threshold to dictionary
        results_dict[f"detection_results"] = detection_results
