    ...
    for n_pixels, n_lines, n_bands in itertools.product(config["N_PIXELS"], config["N_LINES"], config["N_BANDS"]):
        ...
        if config["rx_baseline"]["include"]:
            rx_baseline_detector = rx_baseline.RXBaseline(n_bands=n_bands,
                                                          ...
                                                          threshold=config["rx_baseline"]["threshold"])
        ...
        if config["rt_ck_rxd"]["include"]:
            rt_ck_rxd_detector = rt_ck_rxd.RT_CK_RXD(n_bands=n_bands,
                                                     ...
                                                     threshold=config["rt_ck_rxd"]["threshold"])
        ...
        if config["rx_bil"]["include"]:
            rx_bil_detector = rx_bil.RX_BIL(n_bands=n_bands,
                                            ...
                                            threshold=config["rx_bil"]["threshold"])
        ...
        # Configure CDLSS-AD detector
            cdlss_ad_detector = cdlss_ad.CDLSS_AD(n_bands=n_bands,
                                                  ...
                                                  threshold=config["cdlss_ad"]["threshold"])
        ...
        # Configure LBL-AD detector
            lbl_ad_detector = lbl_ad.LblAD(n_bands=n_bands,
                                           ...
                                           threshold=config["lbl_ad"]["threshold"],
                                           auto_threshold=config["lbl_ad"]["auto_threshold"])
        ...
        # Configure ERX-R detector
        if config["erx_r"]["include"]:
            erx_r_detector = erx.ERX_R(n_bands=n_bands,
                                       n_pixels=n_pixels,
                                       buffer_len=config["erx_r"]["buffer_len"],
                                       momentum=config["erx_r"]["momentum"],
                                       line_offset=config["erx_r"]["line_offset"],
                                       normalise_md=config["erx_r"]["normalise_md"],
                                       threshold=config["erx_r"]["threshold"])
            detector_dict["erx_r"] = erx_r_detector
        ...
        for name, detector in tqdm(detector_dict.items(), disable=True):
            ...
            avg_processing_time /= config["N_REPEATS"]

            # Save processing frequency for current detector
            speed_dict[name] = avg_processing_time
            print(f"Average processing speed: {1/avg_processing_time} lps.")
