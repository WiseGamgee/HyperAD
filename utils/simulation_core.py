...
class DetectionSimulator:
    ...
    def __init__(self, sampler, detector):
        ...
        self.line_offset = self.detector.line_offset

    def run(self, disable_progress_bar=False, save_results=True):
        """
        :param disable_progress_bar: toggle progress bar on/off
        :param save_results: toggle storing detection results
        :return: anomaly detection results and performance
        """
        # Create empty array to store all anomaly detection results
        if save_results:
            self.detection_results = np.zeros(shape=self.sampler_size)
        else:
            self.detection_results = None
        ...
        for i in tqdm(range(self.lines), disable=disable_progress_bar):
            ...
            if y_pred is not None and save_results is True:
                ...
                self.detection_results[:, i - self.line_offset] = y_pred

        # Calculate average time for detector to process a line
        end_time = timer()
        self.avg_processing_time = (end_time - start_time) / self.lines

        # Return results
        return self.detection_results, self.avg_processing_time
